...
///
/// # Panics
/// Panics if called from a non-main thread (AppKit requirement).
pub(crate) fn convert_to_panel(
    app: &tauri::AppHandle,
    ...
    MainThreadMarker::new().ok_or("convert_to_panel must be called on the main thread")?;
    ...
    let target_class = match panel_type {
        PanelType::Recopy => RawRecopyPanel::class() as *const _,
        PanelType::Preview => RawPreviewPanel::class() as *const _,
    };
